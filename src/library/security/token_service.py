...
from library.config import keycloak_config
...
class TokenService:
    ...
    def get_roles_from_token(self, token: str | Mapping[str, Any]) -> list[Role]:
        ...
        :rtype: list[str]
        """
        token_decoded = self.keycloak.decode_token(token=token) if isinstance(token, str) else token
        logger.debug("token_decoded={}", token_decoded)

        roles: Final[str] = token_decoded["resource_access"][self.keycloak.client_id]["roles"]
